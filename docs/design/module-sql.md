...
| 슬롯 upper 를 둘 디스크 | 8-슬롯 sql 의 케이스는 CTP 직렬의 4.04배 걸린다. upper 가 있는 `/var/tmp`(sdc, SATA SSD, swap 도 여기)의 4 KB 동기 쓰기는 5.8 ms, CTP 의 DB 가 있는 `/data`(sdb)는 1.3 ms. *(2026-09-11 측정)* `/data` 로 옮기면 1,131 → 722 s, overlay 를 `volatile`(끝나면 버리는 층이라 sync 를 건너뛴다)로 올리면 338–394 s — CTP 직렬의 4.5–5.2배, 케이스 시간은 CTP 직렬의 절반. medium 직렬도 77 s 로 CTP(87–90 s)보다 빠르다. **남은 결정:** `volatile` 을 슬롯 기본으로 할지(코드에는 아직 없다), 기본 슬롯 루트, 8슬롯의 메모리(한 run 이 메모리 부족으로 중단), 슬롯 동시 기동 — `evidence/sql-native.md` §3 |
