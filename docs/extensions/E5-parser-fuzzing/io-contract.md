...
ctp.sh fuzz-harness [-c <fuzz.conf>] [--target parser|cci|jdbc] [--corpus <dir>]
   또는
testkit run fuzz [--target <name>] [--time <sec>] [--max-len <bytes>] [--fuzzer libfuzzer|afl|honggfuzz]
```
...
| 키 (의제) | 값 | 출처 |
|---|---|---|
| `target` | `parser \| cci \| jdbc \| ...` | fuzz target layer |
...
C-015 cross-cutting 으로 양 측 contract 동시 동결.
