...
- infinite recursion
- binary protocol parser corruption (CUBRID 의 CCI / JDBC 프로토콜 진입점)
...
**잡는 버그 종류:** parser crash / heap overflow / stack corruption / infinite recursion / protocol parser corruption.
...
```
ctp.sh fuzz-harness [-c <fuzz.conf>] [--target parser|cci|jdbc] [--corpus <dir>]
   또는
testkit run fuzz [--target <name>] [--time <sec>] [--max-len <bytes>]
```
...
1. **fuzz target layer 선택** — lexer / parser / binder / planner / executor / CCI protocol / JDBC protocol 중 하나 이상
...
| 책임 경계 | testkit = corpus + replay + triage / cubrid = fuzz target build | C-015 (roadmap cross-cutting 후보) |
...
2. **fuzz target layer 1차 선정** — SQL parser 만 / + CCI protocol / + JDBC protocol
...
6. **C-015 (roadmap cross-cutting)** — survey §13: testkit §6a-E5 × engine-suite (C-004 연속선) — fuzz target build 책임 위치
