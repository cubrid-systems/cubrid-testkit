...
| 022 | 토폴로지는 누가 세우나 — `cubrid-cluster-sandbox` 를 소비한다. **트리거 발화, 본문 미작성 (2026-09-20)**: HA 는 master/slave 가 필요하고 ADR-014 는 *"the system under test has a topology; the runner does not have a fleet"* 라 했다. 그 토폴로지를 세우는 일은 `cluster-sandbox` 에 있고, 이쪽은 subprocess + `--json` 으로 소비한다(ADR-001 C4). `describe --format ctp` 가 CTP 의 동결 키로 렌더되므로 파서도 모델도 새로 만들지 않는다 — 바뀌는 것은 transport 뿐이고, 그것이 `internal/sandbox` 의 Channel 이다. submodule 은 `extensions/cluster-sandbox`, E-번호는 주지 않는다(환경 제공자이지 테스트 능력이 아니다 — `category/extensions/README.md`) | 발화 | `internal/sandbox`, cluster-sandbox ADR-002 |
