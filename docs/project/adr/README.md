...
| [021](ADR-021-crash-reports.md) | 서버가 시그널로 죽은 것을 누가 보나. **초안 (Proposed, 2026-09-17)**: CTP 의 코어 판정(`core.*` 파일 + `FATAL ERROR`)은 core_pattern 이 apport 등으로 가는 환경에서 아무것도 못 본다. 엔진이 직접 쓰는 `$CUBRID/log/coredump/*.coredump` 를 케이스마다 읽고, 처음 보는 리포트가 있으면 **그 케이스를 NOK** 로 만들고 리포트를 run 디렉터리에 보관. CTP 와 판정이 달라지는 것을 의도적으로 받아들인다(ADR-018 규칙 2). 실제로 `reorganization_select_01` 은 여섯 번의 전체 run 에서 매번 서버를 죽이면서 평범한 diff 실패로 보고되어 왔다 (CBRD-27407) | 사용자 검토 | `evidence/isolation-always-failing.md` §1, `evidence/ctp-improvements.md` J |
