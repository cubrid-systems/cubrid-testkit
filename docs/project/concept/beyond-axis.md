...
### B-T16. HA that tests what it is actually for — **P7 shipped, rest of A ready, B blocked**
...
| Group A — beyond | P1 a wait that is a poll on state, not a sleep. **P7 — shipped 2026-09-21**: `testkit check-cases` reads a corpus against its helper library and reports the cases that cannot fail. First run found **7 misspelt verdict calls and 3 self-comparisons in the shell corpus's 3,475** and 1 in the HA corpus's 373; every one read back against the source, all but one of the typos sitting in the `else` branch that was the only line that would have reported the failure. P2's setup half — a transition waited for rather than slept through, which matters because **263 of 373 cases move the topology as setup** and a setup transition that did not finish leaves the comparison after it measuring nothing and reporting green |
| Group A — evidence | the corpus's 20,370 seconds fall to what the engine actually takes, **with no verdict changing** against the two-machine baseline; and every case that cannot reach its own failure path is named. **P1 measured 2026-09-21 on four cases**: seven sleeps became `wait_for_slave`, four verdicts unchanged, 389 s to 316 s, the wait costing ~1.3 s where the sleep cost 5–30 (`evidence/ha/p1-sleep-to-wait.md`) |
...
| Status A | **P7 shipped** — it needed no baseline, because a case that cannot fail is wrong against any engine. P1 is **ready** once the baseline exists; its mechanism is built (`case_patch_dir`, the guard, the `PATCHED` report) and the patch set has a home (`cubrid-testkit-patches`) |
