...
> **축이 셋이다 (2026-09-03, ADR-015).** 이 문서는 축 T / 축 O 의 경계를 다룬다. 둘 다
> **기존 기능 호환성을 지키는 재작성**이고, 성능과 구조는 나아지되 관측 가능한 계약은 움직이지
> 않는다. CTP 에 대응물이 없는 개선 — 기능·성능·사용성 — 은 **축 B** 이고 등록부는
> [`beyond-axis.md`](beyond-axis.md) 다. 여기서 "제외" 된 것이 저기서 "나중에 더 좋게" 로
> 나타나는 경우가 있다 (`FeedbackDB` → B-O2).
