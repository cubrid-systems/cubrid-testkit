...
| **병렬 슬롯** | 직렬 — 6,772 케이스, 케이스당 wall 약 1.4 s (측정 중, `isolation-baseline.md` §4) | 슬롯 1 과 N 의 판정·`result/<name>.log` 동일. wall | `parallel_slots=1` |
...
| **게이트** | ADR-018 확정, 전체 코퍼스 슬롯 1개로 CTP 와 비교 | ADR-018 의 규칙 → 병렬 기본값 on 가능. **초안의 규칙을 현재 데이터에 적용하면 (2026-09-15):** 러너 파일 동일, 러너 차이 0, 불안정 10, 늘 실패 8 — ADR 검토 대기 |
