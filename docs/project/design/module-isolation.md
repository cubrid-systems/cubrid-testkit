...
둘레 — 탐색·제외·큐·슬롯·판정·기록 — 는 Go. 슬롯은 처음부터 구조이고 **기본값이다** (2026-09-15,
사용자 결정 — ADR-018 의 규칙으로 슬롯 1개와 4개 모두 러너 차이 0). 몇 개인지는 고정 4개에서 **기계가 스스로 재는 것**으로
바뀌었다 (ADR-020, 2026-09-16 — 사용자 요청 "그 환경에서는 내 실측치와 다를 수 있으니까 adaptive 하게"): 가용 메모리에서
2 GB 를 뺀 것을 그 기계의 지난 run 이 잰 슬롯당 비용(+15%)으로 나눈 만큼, CPU 하나에 슬롯 하나까지, 그리고 처음에는 4개에서
시작해 run 마다 그 기계가 돌려본 최대의 2배까지 — 같은 코퍼스·같은 레인에서 더 많은 슬롯이 빠르지 않았던 지점(knee)에서 멈춘다.
처음 도는 기계는 슬롯당 1,750 MB. 측정은 체크아웃 밖 `~/.local/state/testkit/sizing/isolation.json` 에 남고, `parallel` 로
conservative·measured·aggressive 를 고른다. `parallel_slots` 를 쓰면 그 값, `1` 이면 CTP 처럼 직렬.
...
**측정 (2026-09-15, `isolation-baseline.md` §4 "Where an attempt's time goes"):** `clean.sh` 는 시도당 115.5 ms, 전체
코퍼스 케이스 시간의 7.1% (790 s). 스냅숏 복원은 한 번에 3,083 ms 라 +20,307 s (+183%) — **기각**, `clean.sh` 는 그대로
둔다. 같은 drop 을 `csql` 세션 하나로 모아도 최대 616 s (5.6%) 라 실행부를 바꿀 값이 없다.

**새 후보 — `qactl` 의 고정 sleep:** 시간의 87–90% 는 `qactl` 이고, 그 가운데 연결 전 `sleepms(100)` 과 클라이언트가
`quit;` 할 때마다 `kill_aclient` 의 `sleepms(100)` 이 전체 케이스 시간의 21.8% (2,417 s) 다. 두 번째는 이미 종료한
클라이언트를 100 ms 기다렸다가 거두는 것이라, 먼저 `waitpid(WNOHANG)` 으로 거두고 살아 있을 때만 원래 경로를 타면
행동이 같다. sandbox 시제품 (`tk-fast/CTP`) 으로 표본 58/58 이 CTP 와 판정·결과 모두 동일, `qactl` 시간 −23%. 전체 코퍼스 슬롯 4개:
wall 2,977 → **2,520 s (−15.4%)**, CTP 단독의 4.4배, 결과 6,755/6,772 가 CTP 와 바이트 동일, ADR-018 규칙으로 러너 차이
0 — 새로 실패한 2건 (`insert_select_05_5` 의 스냅숏 경쟁, `createindex_02` 의 카탈로그 행 순서) 은 단독 3회씩 두 트리
모두 첫 시도에 통과했고, 앞의 것은 CTP 두 run 에서도 첫 시도에 같은 diff 로 졌다. 이것은
