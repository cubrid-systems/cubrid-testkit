...
| JUnit 리포트의 `]]>` | CQT 는 실패 페이로드를 `XMLStreamWriter.writeCData` 에 그대로 넘기고, JDK 8 의 writer 는 `]]>` 를 끊지 않는다 — 측정: `writeCData("before ]]> after")` 가 `<![CDATA[before ]]> after]]>` 로 나온다. 실패한 케이스의 출력에 그 문자열이 있으면 CTP 도 우리도 파서가 읽지 못하는 리포트를 낸다. 바이트 동일성이 우선이라 여기서 고치지 않는다 — upstream(CQT)의 몫 |
...
| 슬롯 upper 를 둘 디스크 | 8-슬롯 sql 의 케이스는 CTP 직렬의 4.04배 걸린다. upper 가 있는 `/var/tmp`(sdc, SATA SSD, swap 도 여기)의 4 KB 동기 쓰기는 5.8 ms, CTP 의 DB 가 있는 `/data`(sdb)는 1.3 ms. *(2026-09-11 측정)* `/data` 로 옮기면 1,131 → 722 s, overlay 를 `volatile`(끝나면 버리는 층이라 sync 를 건너뛴다)로 올리면 338–394 s — CTP 직렬의 4.5–5.2배, 케이스 시간은 CTP 직렬의 절반. medium 직렬도 77 s 로 CTP(87–90 s)보다 빠르다. **결정 (사용자, 2026-09-11):** `volatile` 은 켜는 옵션 — `TESTKIT_SLOT_VOLATILE=1`, 기본은 CTP 와 같은 sync. *(4슬롯 재측정)* `volatile` 이어도 느린 디스크는 느리다 — `/var/tmp` 572 s, `/data` 362 s. 슬롯 루트는 기계의 빠른 디스크에 (`TESTKIT_SLOT_ROOT`, 기본은 `/var/tmp` 그대로). 4슬롯이 8슬롯만큼 빠르다 — 슬롯이 하나씩 27 s 에 올라오는 동안 케이스는 모두 합쳐 780–894 s 라서. sql 슬롯 하나는 정점 2.6 GB (서버 1.38, PL 0.58, 실행부 0.52, CAS 0.12), 러너의 몫은 실행부뿐. *(동시 기동)* `volatile` 이면 슬롯을 함께 올린다 — 기동은 디스크가 아니라 기다림이라 4슬롯이 27 s 에 다 뜨고, sql 4슬롯 322 s (CTP 직렬의 5.5배). `volatile` 이 아니면 하나씩 그대로. medium 은 함께 올려도 77 s(직렬과 같음)에 sesnsch 순서 의존 3건이 드러난다 → **medium 은 직렬** — `evidence/sql-native.md` §3 |
