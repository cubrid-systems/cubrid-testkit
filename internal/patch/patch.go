...
//	  -> $TESTKIT_PATCHES/shell/_06_issues~_17_1h~cbrd_20760_1.patch
//
// Flat rather than a mirror of the corpus, because the corpus is five levels
// deep and this set is not: `ls $TESTKIT_PATCHES/shell` should show everything a run
...
type Set struct {
	// dir is the category root, e.g. $TESTKIT_PATCHES/shell.
	dir string
...
func (p *Set) Report(dir string) error {
	...
	b.WriteString("# Their verdicts are about the patched case. See the README in cubrid-testkit-patches.\n")
	for _, c := range names {
		fmt.Fprintf(&b, "%s\t%s\n", c, p.applied[c])
	}
	n := len(names)
