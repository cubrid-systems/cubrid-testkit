...
| `_06_issues/_12_1h/bug_bts_6159` | rewrites `exec_sql.exp`. The case requires `...skipping` -- what `more` prints when a queued command's characters reach the pager instead of csql, which is the behaviour BTS-6159 is about. The script sent both queries at once and then waited for `csql>`, a prompt the pager suppresses, so whether the keystrokes met the pager was left to how fast the machine drew 200 rows. Measured: passes in the CI image, fails on an Ubuntu host, same engine and corpus. Now the second query is sent only after the pager has said it is showing. |
| `_06_issues/_12_2h/bug_bts_8456` | rewrites `PrintInfo.exp`. It waited for `HASH AT 4`, a string inside the XASL cache dump, and expect carries on after a timeout; and it sent `;sc t_069` without waiting for a prompt. Measured, the log ends `--More--[K csql> 9` -- `;sc t_06` went to the pager and only `9` reached csql, so the schema line the case greps for was never printed. Waits on the session rather than the data, and types nothing until csql answers. |
| `_35_cherry/issue_21654_server_side_loaddb/tran_info` | watches a running `loaddb`, and could miss it entirely: the wait slept two seconds **before** looking, so a load that finished inside that first sleep was never seen -- `$pid` empty, `kill -9` with no argument, every check NOK. Measured: `load.log` says 1,000,000 objects inserted, `tranlist.log` says "There are no transactions". Looks before sleeping, polls at 0.2 s, takes the pid while the process is known alive, and guards the kill. What it tests is unchanged. |
| `_06_issues/_10_2h/bug_xdbms913` | `backuptime_lv3` is a recovery boundary taken with `date +%T`, one-second resolution, and the insert that must fall after it takes under a second on a fast machine -- `restoredb --up-to-date` then includes the rows the boundary was meant to exclude. Measured: wanted 10000, got 20000, exactly the batch just written. The case already follows lv0, lv1 and lv2 with `sleep 3` for this reason; this adds the same sleep to the two lv3 that are followed by a write. |
